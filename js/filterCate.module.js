...
export class FilterCateogry {
    ...
    async getFilterCateogry(arrData,NameMl) {
     
      let response = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${NameMl}`)
      let dataMeals = await response.json();
      arrData = dataMeals.meals;
      console.log(arrData)
      this.displayFilterCateogry(arrData);
      this.getOver();
    
    }

   
    displayFilterCateogry(arrData) {
        let divs = "";
