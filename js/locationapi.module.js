...
export class Arealocation {
    ...
    async getArealocation(arrData,NameMl) {
      
      let response = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${NameMl}`)
      let dataMeals = await response.json();
      arrData = dataMeals.meals;
      console.log(arrData);
      this.displayArealocation(arrData);
      this.getOver();
    }

    displayArealocation(arrData) {
        let divs = "";
